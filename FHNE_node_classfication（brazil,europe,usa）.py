        ...
        if index==0:
            for node in jump_matrix.keys():
                if node in multi_simi[index+1]:
                    jump_matrix[node]['jump_up']=multi_simi[index+1][node]['aver']/0.5
                    jump_matrix[node]['sum'] +=jump_matrix[node]['jump_up']
        elif index==len(multi_simi)-1:
            for node in jump_matrix.keys():
                if node in multi_simi[index-1]:
                    jump_matrix[node]['jump_down']=multi_simi[index-1][node]['aver']/5
            ...
            while True:
                ...
                while True:
                    if next_node=='jump_down':  #节点要跳转到下一层
                        ...
                        break
                t=random.uniform(multi_simi[k_th][node_seq[-1]]['min']/(k_th+1),multi_simi[k_th][node_seq[-1]]['max'])
...
if __name__=="__main__":
    #brazil参数设置：layer=4，p=1
    #europe参数设置：layer=4，p=1
    #usa参数设置：layer=4,p=1
    dir1,dir2,dir3='../graph/','../random/','../../对比实验/source_emb/'
    edgefile="usa-flights.edgelist"
    randomfile='random.csv'
    trainsave='usa_fhne.emb'
    layer=4
    p=1
    all_nodes,multi_simi,layer_nodes,k=k_core_mgnrl(dir1+edgefile,layer,p,0.9,0.1)  #标准
